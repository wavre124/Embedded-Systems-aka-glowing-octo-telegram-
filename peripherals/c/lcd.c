...
)
{
  ...
  y1 = y_start  + (image_height_pixels/2);
	...
	for(i = 0; i < image_width_pixels; i++) {
		for(j = 0; j < image_height_pixels; j++) {
			lcd_write_data_u16(bColor);
		}
	}
...
)
{
	...
	lcd_set_pos(x_start, x_start + 2, y_start, y_start - image_height_pixels + 1);
